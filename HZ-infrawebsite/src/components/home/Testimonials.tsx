...
export function Testimonials() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const res = await api.get<Partial<typeof DEFAULT>>('/site-config/testimonials', { signal: ac.signal });
        setCms((prev) => ({
          ...prev,
          ...res.data,
          items: res.data.items?.length ? res.data.items : prev.items,
        }));
      } catch {
        /* defaults */
      }
    ...
    <section className="overflow-x-hidden bg-white py-9 md:py-14">
      <div className="mx-auto max-w-infra px-4 md:px-7">
        <div className="mb-8 text-center md:mb-8">
          ...
          <h2 className="mt-1.5 font-montserrat text-[26px] font-extrabold text-charcoal">{cms.title}</h2>
        </div>

        <div className="grid grid-cols-1 gap-4 md:grid-cols-3 md:gap-[18px]">
          {cms.items.map((q) => (
            <figure key={q.name} className="rounded-[14px] border border-[#dde8f5] bg-offwhite p-5 md:p-[22px]">
