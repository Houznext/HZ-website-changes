...
export async function uploadBrowseCityImage(file: File): Promise<{ url?: string }> {
  const token = getToken();
  const fd = new FormData();
  fd.append('file', file);
  const res = await fetch(`${publicBase()}/upload/browse-city-image`, {
    method: 'POST',
    headers: token ? { Authorization: `Bearer ${token}` } : {},
    body: fd,
  });
  if (!res.ok) throw new Error('Browse city image upload failed');
  return res.json();
}

export async function uploadBrowseTypeImage(file: File): Promise<{ url?: string }> {
  const token = getToken();
  const fd = new FormData();
  fd.append('file', file);
  const res = await fetch(`${publicBase()}/upload/browse-type-image`, {
    method: 'POST',
    headers: token ? { Authorization: `Bearer ${token}` } : {},
    body: fd,
  });
  if (!res.ok) throw new Error('Browse type image upload failed');
  return res.json();
}
...
export async function uploadHeroImage(file: File): Promise<{ url?: string }> {
  ...
  const fd = new FormData();
  fd.append('file', file);
  const res = await fetch(`${publicBase()}/upload/hero-image`, {
    method: 'POST',
    headers: token ? { Authorization: `Bearer ${token}` } : {},
    body: fd,
  });
